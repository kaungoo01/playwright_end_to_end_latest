//If API session is too complex
//Login One time
//Store all the info in json file
//Next time inject all of it.
//Reuse from end2endTest_Office
...
//New: Before All > Create a Session Storage.
...
test('Test 1: Client Login , Place Order, and Validate the OrderID', async () => {
   ...
   //Title
    ...
    //await expect(page.locator(".user__name [type='text']").first()).toHaveText(email); //.user__name [type='text'] two location, .first will give the 1st item.
...
test('Test 2: Check some text', async () => {
        const page = await webContext.newPage();
        //go to this page
        await page.goto("https://rahulshettyacademy.com/client");
        //Title
...
test('Test 3: CHeck more text',async ()=>{
    const page = await webContext.newPage();
    //go to this page
    await page.goto("https://rahulshettyacademy.com/client");
    const pageTitle = (await page.title());
